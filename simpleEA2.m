...
%% TODO
cross_Prob = fitness./sum(fitness); % roulette-wheel selection
offspringGenes = [];
for i = 1:populationSize/2
    ...
    for j = 1:2
        ...
        for index = 1:populationSize
            if r > sum(cross_Prob(1:index-1)) && r <= sum(cross_Prob(1:index))
                break;
            end
        end
...
population = bin2dec(genotypes);  %decode
fitness = objective(population);
A = max(fitness);
fitness_pop=[fitness_pop, A]
